...
namespace Monsta
{
    ShaderSystem::ShaderSystem(const std::string& src) : m_Source(src) { }
    ShaderSystem::~ShaderSystem() = default;

    std::tuple<bool, unsigned int> ShaderSystem::compile_shader(const ShaderType& shader_type) noexcept
    {
    ...
    bool ShaderSystem::link_shader(const unsigned int& shader_pid, const unsigned int& shader_id) noexcept
    {
        GLint did_succeed;
    ...
    const std::string& ShaderSystem::get_source() const noexcept
    {
        return this->m_Source;
    }

    unsigned int ShaderSystem::get_shader_pid() noexcept
    {
        return glCreateProgram();
    }

    void ShaderSystem::destroy_shader_pid(const unsigned int& pid) noexcept
    {
        glDeleteProgram(pid);
    }
