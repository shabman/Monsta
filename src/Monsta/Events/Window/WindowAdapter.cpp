...
void
WindowAdapter::RegisterWindowFocusEvent ( GLFWwindow *window, int isFocused ) noexcept
{
...
bool
WindowAdapter::already_exists ( const WindowListener *listener ) noexcept
{
  return std::find ( s_Events.begin (), s_Events.end (), listener ) != s_Events.end ();
}

void
WindowAdapter::add_listener ( const WindowListener *listener ) noexcept
{
...
void
WindowAdapter::remove_listener ( const WindowListener *listener ) noexcept
{
  if ( already_exists ( listener ) )
...
bool
WindowAdapter::has_listener ( const int &index ) noexcept
{
...
WindowAdapter::get_listener ( const int &index ) noexcept
{
  if ( index > s_Events.size () )
    return nullptr;
